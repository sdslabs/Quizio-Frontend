...
const SideNavOption = ({
 SelectedIcon, Icon, text,
  ...
  return (
      <button type="button" onClick={handleChangeSet} className={`create-quiz-sidenav-option${isActive ? '-selected' : ''}`}>
          <div className="create-quiz-sidenav-option-icon">{isActive ? <SelectedIcon /> : <Icon />}</div>
          <div className={`create-quiz-sidenav-option-text${isActive ? '-selected' : ''}`}>{text}</div>
      </button>
  );
};
